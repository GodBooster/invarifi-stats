...
const getJoeBoostedLpApys = async () => {
  ...
  for (let i = 0; i < pools.length; i++) {
    ...
    const performanceFee = getTotalPerformanceFeeForVault(pool.name) + 0.05; //  fees + 0.05 Joe Boost Tax
    const shareAfterPerformanceFee = 1 - performanceFee;

    const vaultApr = simpleApy.times(shareAfterPerformanceFee);
    const vaultApy = compound(simpleApy, BASE_HPY, 1, shareAfterPerformanceFee);
