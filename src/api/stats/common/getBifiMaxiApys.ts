...
export const getBifiMaxiApys = async (params: BifiApyParams) => {
  ...
  const shareAfterPerformanceFee = 1 - getTotalPerformanceFeeForVault(params.chain + '-bifi-maxi');
  const apy = compound(simpleApy, DAILY_HPY, 1, shareAfterPerformanceFee);
