...
const getPoolsData = async params => {
  ...
  const [
    ...
    Promise.allSettled(balanceCalls),
    Promise.allSettled(rateCalls),
    Promise.allSettled(depositBalanceCalls),
    Promise.allSettled(derivedBalanceCalls),
    Promise.allSettled(periodFinishCalls),
    Promise.allSettled(rewardRateCalls),
    Promise.allSettled(rewardDataCalls),
  ]);

  const balances = balanceResults.map(
    v => new BigNumber(v.status === 'fulfilled' ? v.value.toString() : '0')
  );
  const rates = rateResults.map(
    v => new BigNumber(v.status === 'fulfilled' ? v.value.toString() : '0')
  );
  const depositBalances = depositBalanceResults.map(
    v => new BigNumber(v.status === 'fulfilled' ? v.value.toString() : '0')
  );
  const derivedBalances = derivedBalanceResults.map(
    v => new BigNumber(v.status === 'fulfilled' ? v.value.toString() : '0')
  );
  const periodFinishes = periodFinishResults.map(v =>
    v.status === 'fulfilled' ? v.value.toString() : '0'
  );
  const rewardRateFlat = rewardRateResults.map(
    v => new BigNumber(v.status === 'fulfilled' ? v.value.toString() : '0')
  );
  const rewardDataFlat = rewardDataResults.map(
    v => new BigNumber(v.status === 'fulfilled' ? v.value.toString() : '0')
  );
