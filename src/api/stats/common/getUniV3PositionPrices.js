...
import UniswapPositionHelperAbi from '../../../abis/UniswapPositionHelper';
...
const getPoolData = async params => {
  const helperContract = fetchContract(params.helper, UniswapPositionHelperAbi, params.chainId);
  const calls = params.pools.map(pool =>
    helperContract.read.getPositionTokens([pool.nftId, pool.address])
  );
