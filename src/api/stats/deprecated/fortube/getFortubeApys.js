...
const getFortubeApys = async () => {
  ...
  try {
    ...
    Object.values(dataSimple).map(item => {
      ...
      const shareAfterPerformanceFee = 1 - getTotalPerformanceFeeForVault(`fortube-${symbol}`);
      const apy = compound(parseFloat(item.estimated_ar), WEEKLY_HPY, 1, shareAfterPerformanceFee);
