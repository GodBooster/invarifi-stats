...
async function performUpdateAmmPrices() {
  ...
  const ammPrices = fetchAmmPrices(pools, knownPrices)
    ...
    .catch(() => ({ tokenPrices: {}, poolPrices: {}, lpsBreakdown: {} }));

  const venusPrices = ammPrices
    .then(async ({ tokenPrices }) => {
      return await fetchVenusPrices(tokenPrices);
    })
    .catch(() => ({}));

  const curveTokenPrices = ammPrices
    .then(async ({ tokenPrices }) => {
      return await fetchCurveTokenPrices(tokenPrices);
    })
    .catch(() => ({}));

  const concentratedLiquidityTokenPrices = ammPrices
    .then(async ({ tokenPrices }) => {
  ...
  const xPrices = ammPrices
    .then(async ({ tokenPrices }) => {
      return await fetchXPrices(tokenPrices);
    })
    .catch(() => ({}));

  const mooPrices = ammPrices
    .then(async ({ poolPrices, tokenPrices }) => {
      return await fetchMooPrices(mooTokens, tokenPrices, poolPrices);
    })
    .catch(() => ({}));

  const optionPrices = ammPrices
    .then(async ({ tokenPrices }) => {
      return await fetchOptionTokenPrices(tokenPrices);
    })
    .catch(() => ({}));

  const linearPoolPrice = ammPrices
    .then(async ({ tokenPrices }): Promise<Record<string, number>> => {
  ...
  const lpData = ammPrices
    .then(async ({ poolPrices, lpsBreakdown }) => {
      const dmm = await dmmPrices;
...
async function updateAmmPrices() {
  ...
  try {
    ...
    const [tokenPrices, lpPrices, lpBreakdowns] = await Promise.allSettled([
      tokenPricesPromise,
      lpPricesPromise,
      lpBreakdownPromise,
    ]);
    ...
    if (
      addToCache(
        tokenPrices.status === 'fulfilled' ? tokenPrices.value : {},
        lpPrices.status === 'fulfilled' ? lpPrices.value : {},
        lpBreakdowns.status === 'fulfilled' ? lpBreakdowns.value : {}
      )
    ) {
      await saveToRedis();
    }
