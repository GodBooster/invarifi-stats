...
const updateBoostAprsForChain = async (chain: ApiChain, boosts: Boost[]) => {
  ...
  const totalSupplyCalls = boosts.map(boost => {
    const contract = fetchContract(boost.earnContractAddress, BoostAbi, chainId);
  ...
  try {
    const [totalSupply, rewardRate, periodFinish] = await Promise.all([
      Promise.allSettled(totalSupplyCalls),
    ...
    for (let i = 0; i < boosts.length; i++) {
      ...
      const rate = rewardRate[i];
      const finish = periodFinish[i];
      const apr = await mapResponseToBoostApr(
        boosts[i],
        supply.status === 'fulfilled' ? supply.value : BigInt(0),
        rate.status === 'fulfilled' ? rate.value : BigInt(0),
        finish.status === 'fulfilled' ? finish.value : BigInt(0)
      );
