...
let assetsByChain: TreasuryAssetRegistry = keysToObject(SUPPORTED_CHAINS, () => ({}));

let tokenBalancesByChain: TreasuryBalances = keysToObject(SUPPORTED_CHAINS, () => ({}));

let treasurySummary: TreasuryReport = keysToObject(SUPPORTED_CHAINS, () => ({}));

function updateTreasuryAddressesByChain() {
  treasuryAddressesByChain = keysToObject(SUPPORTED_CHAINS, chain => {
    const chainAddressbook = addressBook[chain];
    const addresses = {};
    const treasuryMultisig = chainAddressbook.platforms.cubera.treasuryMultisig;
    const treasury = chainAddressbook.platforms.cubera.treasury;
...
function updateAssetsByChain() {
  ...
  assetsByChain = keysToObject(SUPPORTED_CHAINS, chain => {
    return {
      ...(tokenAssets[chain] || {}),
      ...(vaultAssets[chain] || {}),
    };
...
function getTokenAddressesByChain(): TreasuryAssetRegistry {
  return keysToObject(SUPPORTED_CHAINS, chain => {
...
async function updateTreasuryBalances() {
  try {
    ...
    await Promise.allSettled(SUPPORTED_CHAINS.map(updateSingleChainTreasuryBalance));
...
async function buildTreasuryReport() {
  const chainReports = await Promise.all(SUPPORTED_CHAINS.map(buildTreasuryReportForChain));
  for (const i in SUPPORTED_CHAINS) {
    treasurySummary[SUPPORTED_CHAINS[i]] = chainReports[i];
  }
}
...
export function getTreasury(): TreasuryReport {
