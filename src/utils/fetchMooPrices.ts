...
import VaultV6Abi from '../abis/Vault';
...
const fetchPpfs = async (pools: any[]) => {
  ...
  for (let i = 0; i < uniqueChainIds.length; i++) {
    ...
    const ppfsCalls = filtered.map(pool => {
      const contract = fetchContract(pool.address, VaultV6Abi, uniqueChainIds[i]);
...
export const fetchChainVaultsPpfs = async (vaults: any[], chain) => {
  ...
  const contracts = vaults
    ...
    .map((address: `0x${string}`) =>
      fetchContract<typeof VaultV6Abi>(address, VaultV6Abi, parseInt(chainId))
    );

  const ppfs = await Promise.all(
    contracts.map(async c => {
      return await c.read.getPricePerFullShare();
    })
  );
